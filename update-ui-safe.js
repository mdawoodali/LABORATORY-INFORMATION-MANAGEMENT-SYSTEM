...
fields.forEach(f => {
    // Instead of regex, use simple string replace
    const strToFind = `<label className="text-xs font-semibold text-slate-600">${f.oldLabel}</label>
                  <DropdownInput value={formData.${f.key}} onChange={v => updateField('${f.key}', v)} fieldKey="${f.oldFieldKey}" className="w-full border rounded p-2 text-sm" />`;
    ...
    // We can just use split and join if the whitespace is exactly matching.
    // If not, we can use a relaxed regex.
    const relaxedRegex = new RegExp('<label className="text-xs font-semibold text-slate-600">' + escapeRegex(f.oldLabel) + '</label>[\\s\\S]*?<DropdownInput value=\\{formData\\.' + escapeRegex(f.key) + '\\} onChange=\\{v => updateField\\(\\'' + escapeRegex(f.key) + '\\', v\\)\\} fieldKey="' + escapeRegex(f.oldFieldKey) + '" className="w-full border rounded p-2 text-sm" />');
    
    code = code.replace(relaxedRegex, replacement);
});
...
itemFields.forEach(f => {
    const relaxedRegex = new RegExp('<DropdownInput fieldKey="' + escapeRegex(f.key) + '" value=\\{item\\.' + escapeRegex(f.stateKey) + '\\} onChange=\\{val => updateItem\\(item\\.id, \\'' + escapeRegex(f.stateKey) + '\\', val\\)\\} className="w-full border rounded p-1 text-sm([\\s\\S]*?)" placeholder="[\\s\\S]*?" />');
...
// Price and samples
const priceRegex = /<label className="text-\[10px\] text-slate-500">PRICE \(PKR\)<\/label>[\s\S]*?<DropdownInput fieldKey="inv_item_price" placeholder="0" value=\{item\.price\?\.toString\(\) \|\| ''\} onChange=\{val => updateItem\(item\.id, 'price', val\)\} className="w-full border rounded p-1 text-sm text-right" \/>/;
...
const samplesRegex = /<label className="text-\[10px\] text-slate-500">No of sample<\/label>[\s\S]*?<DropdownInput fieldKey="inv_item_samples" placeholder="1" value=\{item\.samples\?\.toString\(\) \|\| ''\} onChange=\{val => updateItem\(item\.id, 'samples', val\)\} className="w-full border rounded p-1 text-sm text-right" \/>/;
